...
@app.get("/api/frequencies/official-titles/search")
def search_official_title(
    word: str = Query(..., description="官称词汇"),
    era: Optional[str] = Query(None, description="时代过滤器"),
    db: Session = Depends(get_db),
):
    """
    Search for a specific official title and return its occurrence count.

    - **word**: The official title to search for
    ...
    # 查找匹配的官称
    matched_pair = None
    for display_name, variants in all_title_pairs:
        if word in variants or word == display_name:
    ...
    transcripts = get_all_transcripts(db, era=era)
    if not transcripts:
        return {"word": display_name, "count": 0, "variants": variants, "era": era}
